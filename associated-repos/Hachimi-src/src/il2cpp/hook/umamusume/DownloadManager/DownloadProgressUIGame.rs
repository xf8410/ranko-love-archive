...
pub fn init(DownloadManager: *mut Il2CppClass) {
    find_nested_class_or_return!(DownloadManager, DownloadProgressUIGame);

    let show_addr = get_method_addr(DownloadProgressUIGame, c"Show", 0);
    let hide_addr = get_method_addr(DownloadProgressUIGame, c"Hide", 1);
    let set_progress_addr = get_method_addr(DownloadProgressUIGame, c"SetProgress", 1);
